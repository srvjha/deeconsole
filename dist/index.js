...
function resolveTraverse(module) {
    ...
    if (maybeDefault && typeof maybeDefault.default === 'function') {
...
function transformSource(code, options, filename) {
    let ast;
    try {
        ast = parse(code, {
        ...
        console.warn(`Skipping ${filename}: failed to parse (${error.message}).`);
...
function isConsoleInvocation(path) {
...
function expandRemovalRange(code, start, end) {
